...
/**
 * Интеграционные тесты для сервиса Kafka, проверяющие функциональность отправки email при удалении пользователя.
 *
 * <p>Тесты используют встроенный Kafka брокер для изоляции тестовой среды и проверяют:
 * <ul>
 *   <li>Корректность отправки событий удаления пользователя</li>
...
class KafkaServiceIntegrationDeleteUserTest {
     ...
     * <ul>
     *   <li>Группа: "test-group-delete"</li>
    ...
    void setUp() {
        Map<String, Object> consumerProps = KafkaTestUtils.consumerProps(
                "test-group-delete", "true", embeddedKafkaBroker);
    ...
    /**
     * Проверяет успешную отправку события удаления пользователя.
     *
     * <p>Тест проверяет, что при вызове метода {@link KafkaService#sendEmailOnUserDelete(UserEntity)}
     * в Kafka отправляется корректное событие с правильными данными:
     * <ul>
     *   <li>Ключ сообщения соответствует email пользователя</li>
     *   <li>Тип события равен {@link EventType#DELETE}</li>
     *   <li>Email и имя в событии соответствуют переданному пользователю</li>
     * </ul>
     * </p>
     */
    @Test
    void sendEmailOnUserDelete_WhenEventSuccess_GetRecord() {
    ...
    /**
     * Проверяет корректную обработку множественных событий удаления пользователей.
     *
     * <p>Тест проверяет, что при отправке нескольких событий удаления:
     * <ul>
     ...
     *   <li>Тип события для всех сообщений равен {@link EventType#DELETE}</li>
     * </ul>
     * </p>
     */
    @Test
    void sendEmailOnUserDelete_WhenEventMultipleSuccess_GetRecordWithProperOrder() {

        UserEntity user1 = new UserEntity("Aa", "aaa@mail.com", 12);
        UserEntity user2 = new UserEntity("Bb", "bbb@mail.com", 12);

        kafkaService.sendEmailOnUserDelete(user1);
        kafkaService.sendEmailOnUserDelete(user2);
        ...
        await().atMost(Duration.ofSeconds(10))
                ...
                .untilAsserted(() -> {
                    ...
                    assertThat(record1.value().getEventType()).isEqualTo(EventType.DELETE);
                    ...
                    assertThat(record2.value().getEventType()).isEqualTo(EventType.DELETE);
     ...
     * {@link KafkaService#sendEmailOnUserDelete(UserEntity)} приводит к выбрасыванию
     * исключения {@link NullPointerException}.
     * </p>
     */
    @Test
    void sendEmailOnUserDelete_WhenUserIsNull_ShouldThrowException() {

        assertThrows(NullPointerException.class,
                () -> kafkaService.sendEmailOnUserDelete(null));
    }
}